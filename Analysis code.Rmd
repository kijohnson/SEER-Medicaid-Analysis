...
seer0to39 <- seer0to39 %>%
  ...
  # ENROLLEES
                        if_else(In_Medicaid_Flag == 1 & status3 == 0, 3, # not enrolled in Medicaid in dx month
...
seer0to39$enroll3_cat3 <- factor(seer0to39$enroll3_cat3, levels = c(0:7), labels = 
                                   c("0: NM Private", 
                                     ...
                                     "4: ME continuous",
                                     "5: Missing ME in dx month/private",
                                     "6: Missing ME in dx month/other",
                                     "7: Missing ME in dx month/uninsured"
                                     ))
...
table(seer0to19$enroll3_cat3, seer0to19$can_dead)
...
```{r}
seer0to39 <- seer0to39 %>%
  mutate(Bradley_def1_3 =case_when(In_Medicaid_Flag == 0 ~0, # Not insured by Medicaid
                                 In_Medicaid_Flag == 1 & (status3 %in% c(0,1,2,3,4) & (pre_mo3 >0 | post_mo3 >0))~ 1, # insured by Medicaid within a 7 mo window
                                 In_Medicaid_Flag == 1 & status3 == 9|is.na(status3)|status3 == 0  ~ 2))  # not insured or unknown insurance by Medicaid within 25 mo
# Fix NAs
...
# Further classify those who are enrolled
seer0to39 <- seer0to39 %>%
  mutate(Bradley_def2_3 = case_when(Bradley_def1_3 == 0 ~ 0, # Not insured by Medicaid (not linked)
                                  Bradley_def1_3 == 2 ~ 1,  # Not insured or unknown Medicaid insurance within 6 months
                                  Bradley_def1_3 == 1 & status3 %in% c(0,1,2) & pre_mo3>=0 & (post_mo3==0|is.na(post_mo3)) ~2, # insured by Medicaid before diagnosis 
                                  Bradley_def1_3 == 1 & status3 %in% c(0,1,3) & (pre_mo3==0|is.na(pre_mo3)) & post_mo3>=0 ~3, # insured by Medicaid after diagnosis 
                                  Bradley_def1_3 == 1 & status3 %in% c(0,1,4) & pre_mo3>0 & post_mo3>0 ~4 # insured by Medicaid before and after dx
                                 ))
# code all NAs as insured by Medicaid only
seer0to39 <- seer0to39 %>% 
  mutate(Bradley_def2_3 = if_else(is.na(Bradley_def2_3), 5, Bradley_def2_3))
                      
seer0to39$Bradley_def2_3 <- factor(seer0to39$Bradley_def2_3, levels = c(0:5), labels = 
                                   c("0: Not linked to Medicaid", 
                                     "1: Not insured or unknown Medicaid insurance within 7 months",
                                     "2: Insured by Medicaid within 3 mo before dx", 
                                     "3: Insured by Medicaid within the 3 months after the dx",
                                     "4: Insured by Medicaid with both the 3 month window before and after dx",
                                     "5: Insured by Medicaid at dx only"
                                     ))

table(seer0to39$insurance, seer0to39$Bradley_def2_3, useNA = "always")
test2 <- seer0to39 %>%
  group_by(In_Medicaid_Flag, insurance, status3, pre_mo3, post_mo3) %>%
...
seer0to39 <- seer0to39 %>%
  mutate(mcont = case_when(Bradley_def2_3 == "0: Not linked to Medicaid" ~ 0,
                                Bradley_def2_3 == "1: Not insured or unknown Medicaid insurance within 7 months" ~1,
                                Bradley_def2_3 %in% c("2: Insured by Medicaid within 3 mo before dx",
                                                      "3: Insured by Medicaid within the 3 months after the dx","5: Insured by Medicaid at dx only") ~2,
                                Bradley_def2_3 ==  "4: Insured by Medicaid with both the 3 month window before and after dx" ~3))

table(seer0to39$mcont)

seer0to39$mcont <- factor(seer0to39$mcont, levels = c(0:3), labels = c("Not linked ot Medicaid", "Not insured or unknown Medicaid insurance within 7 months", "Discontinous Medicaid enrollment", "Continuous Medicaid enrollment"))
...
ggsave(filename = "Enroll3_cat3 KM plot 0 to 19.png",
       width = 11, height = 8.5, units = "in", bg="white") 
...
ggsave(filename = "Enroll3_cat3 KM plot 20 to 39.png",
       width = 11, height = 8.5, units = "in", bg="white") 
```
# Bradley def (sort of anyway)
```{r}
p<-ggsurvplot(fit = survfit(Surv(survmo, can_dead) ~ Bradley_def2_3, data =seer0to19), 
    xlab = "Months from diagnosis", 
    ylab = "Cancer survival proportion",
    legend.title = "",
...
# Mcont def
```{r}
p<-ggsurvplot(fit = survfit(Surv(survmo, can_dead) ~ mcont, data =seer0to19), 
    xlab = "Months from diagnosis", 
    ylab = "Cancer survival proportion",
    legend.title = "",
    #legend.labs = c("0: Not insured by Medicaid within 7 month window", 
...
# adjusted models 20 to 39
res.cox <- coxph(Surv(survmo, can_dead) ~ mcont +race_cat + Age_recode_with_1_year_olds + Census_Tract_Poverty_Indicator, data = seer20to39) # Model adjusted for race age and a measure of poverty
res.cox
summary(res.cox)
