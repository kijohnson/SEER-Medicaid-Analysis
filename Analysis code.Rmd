...
```{r}
seer0to39<-read_csv("/Volumes/kijohnson/Active/seer0to39.csv", show_col_types = FALSE)
enroll_seerids<-read_csv("/Volumes/kijohnson/Active/enroll_seerids.csv", show_col_types = FALSE)
...
seer0to39 <- seer0to39 %>%
  mutate(Bradley_def1_3 =case_when(In_Medicaid_Flag == 0 ~0, # Not insured by Medicaid
                                 In_Medicaid_Flag == 1 & (status3 %in% c(0,1,2,3,4) & (pre_mo3 >0 | post_mo3 >0))~ 1, # insured by Medicaid within a 25 mo window
...
# Further classify those who are enrolled
seer0to39 <- seer0to39 %>%
  mutate(Bradley_def2_3 = case_when(Bradley_def1_3 == 0 ~ 0, # Not insured by Medicaid within 6 months
...
seer0to39$Bradley_def2_3 <- factor(seer0to39$Bradley_def2_3, levels = c(0:5), labels = 
                                   c("0: Not insured by Medicaid within 7 month window", 
